...
import { Clock, AlertCircle, CheckCircle2, User, Edit, Trash2 } from 'lucide-react';

const TaskTable = () => {
    const editTask = (taskId) => {
        const response = api.post(`/tasks/edit/${taskId}`, {
        })
    ...
    const { data: tasks, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            const res = await api.get('/admin/task-details');
            return res.data;
        }
    });
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {tasks.map((task) => (
                            <tr key={task.id} className="hover:bg-gray-50 transition">
                                ...
                                {/* 3. Assigned To (With Designation) */}
                                <td className="p-4">
